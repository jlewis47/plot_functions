...
def lesc_plot(
    fig, ax, masses, lescs, lesc_type, xlabel, redshift=None, log=True, **plot_args
):
    lesc_labels = {
        "gas": "$\mathrm{L_{esc, g}}$",
...
def lesc_Mh_plot(
    fig, ax, masses, lescs, lesc_type, redshift=None, log=True, **plot_args
):
    return lesc_plot(
        fig,
        ax,
...
def plot_ndot(fig, ax, redshifts, lescs, lo=None, hi=None, med=None, **plot_args):
    lines = xy_plot_vect(
        fig,
        ax,
        redshifts,
    ...
    if np.all(lo != None) and np.all(hi != None):
        fill = ax.fill_between(redshifts, lo, hi, alpha=0.3, color=lines.get_color())
    if np.all(med != None):
        med = ax.plot(redshifts, med, color=lines.get_color(), linestyle="--")
...
def plot_dustier_lesc(ax, redshift, fkey="lesc", zprec=0.1, **plot_args):
    ...
    assert (
        fkey == "lesc" or fkey == "lesc_total"
    ), "fkey must be either lesc or lesc_total"

    with h5py.File(
        os.path.join(dir_path, "../constraints/dustier_median_lescs")
    ) as src:
...
def plot_cosmic_ndot_constraints(ax, redshift, zprec=0.5, **plot_args):
    ...
    with h5py.File(
        os.path.join(dir_path, "../constraints/cosmic_ndot_constraints.hdf5"), "r"
    ) as src:
    ...
    b13 = ax.errorbar(
        zbb13,
        10 ** np.float64(lgambb13 + 51),
        yerr=[10 ** (erplgbb13 + 51) - 1e51, -1e51 + 10 ** (-ermlgbb13 + 51)],
        fmt="^",
        color="b",
        mec="b",
        label=r"Becker-Bolton13",
    )

    bw15 = ax.scatter(
        bouwens15a[:, 0],
        10 ** np.float64(bouwens15a[:, 1]),
        marker="p",
        color="navy",
        label="Bouwens+15a from UVLF",
        s=65,
    )
    ...
    (ke19,) = ax.plot(
        keating_19_nphot[0],
        keating_19_nphot[1] * 1e50,
        label="Keating+19",
        linestyle="--",
    )
    (ku19,) = ax.plot(
        kulkarni_19_nphot[0],
        kulkarni_19_nphot[1] * 1e50,
        label="Kulkarni+19",
        linestyle="--",
    )
    (p19,) = ax.plot(
        puchwein19[:, 0],
        10 ** np.float64(puchwein19[:, 1]),
        linestyle="-.",
        color="k",
        label="Puchwein+19 fiducial galaxies",
    )

    return (
        [b13, bw15, ma17, f19, ke19, ku19, p19],
        [
            "Becker-Bolton+13",
            "Bouwens+15a",
            "Madau+17",
