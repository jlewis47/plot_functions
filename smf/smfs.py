        ...
        with open(path,'r') as src:
                ...
                for il,line in enumerate(src):
                        ...
                        if line[0]=='z':
                                ...
                                if len(cur_smf)>0:smfs.append(cur_smf)
                                cur_smf=[]
                        else:
...
def plot_constraints(fig, ax, tgt_zed, zed_prec=0.5, color='k'):
    ...
    zeds_stefanon21, smfs_stefanon21=get_stefanon21_smf(os.path.join(dir_path,'../constraints/stefanon_smf_21'))

    zeds_kiku20, smfs_kiku20=get_stefanon21_smf(os.path.join(dir_path,'../constraints/kikuchihara20_smf'))
    ...
    for set,zed in enumerate(zeds):
        ...
        if np.any(dist<zed_prec):
            ...
            if match_smf.shape[1]>2:
                ...
                xerr = [10**(match_smf[:,0]-match_smf[:,1]), 10**(match_smf[:,0]+match_smf[:,1])]
                yerr = np.asarray([(match_smf[:,2]-match_smf[:,4]), (match_smf[:,3]+match_smf[:,2])])

                line=ax.errorbar(10**match_smf[:,0], match_smf[:,2], xerr=xerr, yerr=yerr,
                    color=color,linestyle='none',capsize=5, marker=markers[set])
                ...
                line=ax.errorbar(match_smf[:,0], match_smf[:,1], xerr=xerr, yerr=yerr,
                    color=color,linestyle='none',capsize=0, marker=markers[set])
