...
def plot_msmd(fig, ax, ms, md, redshift=None, **plot_args):
    line = xy_plot_vect(
        ...
        ms,
        md,
        xlabel="$\mathrm{Stellar \, Mass, \, M_\odot}$",
        ylabel="$\mathrm{Dust \, Mass, \, M_\odot}$",
        xscale="log",
        yscale="log",
        **plot_args
    )
...
def plot_dustier_msmd(ax, redshift, zprec=0.1, **plot_args):
    ...
    with h5py.File(os.path.join(dir_path, "../constraints/dustier_mdms")) as src:
        ...
        redshifts = np.asarray(
            [float(k.split("_")[-1].lstrip("z")) for k in keys if "mdust" in k]
        )
...
def plot_msmd_constraints(ax, redshift, zprec=0.1):
    ...
    burgarella = pd.read_html(
        os.path.join(dir_path, "../constraints/burgarella_2020_tab")
    )[0]
    ...
    for ir, row in enumerate(burg_2020_arr):
        for ic, col in enumerate(row):
            ...
            if isinstance(col, str):
                burg_2020_arr[ir, ic] = col.replace("\u2005\xd7\u200510", "e").replace(
                    " \xd7 10", "e"
                )
    ...
    delphi_dtg_z7 = np.genfromtxt(
        os.path.join(dir_path, "../constraints/dayal22_dtg_z7"), delimiter=","
    )
    delphi_dtm_z7 = np.genfromtxt(
        os.path.join(dir_path, "../constraints/dayal22_dtm_z7"), delimiter=","
    )
    delphi_dmsm_z7 = np.genfromtxt(
        os.path.join(dir_path, "../constraints/dayal22_dmsm_z7"), delimiter=","
    )
    rebels_dust_masses = np.genfromtxt(
        os.path.join(dir_path, "../constraints/dayal22_rebels_dust"),
        delimiter=",",
        skip_header=1,
    )
    ...
    if 6 <= redshift < 7.5:
        manc = ax.errorbar(
            10**mancini_mstar,
            10**mancini_mdust,
            xerr=[10**mancini_err_mstar_dw, 10**mancini_err_mstar_dw],
            yerr=-(10**mancini_mdust) + 10 ** (mancini_mdust + 0.2),
            uplims=True,
            fmt="D",
            mec="navy",
            mfc="none",
    ...
    if 5 <= redshift < 10:
        ...
        burg = ax.errorbar(
            burg_2020_mstar[burg_mask],
            burg_2020_mdust[burg_mask],
            yerr=np.abs(burg_2020_mdust_err - burg_2020_mdust)[burg_mask],
    ...
    rebs = ax.errorbar(
        10 ** rebels_dust_masses[:, 1],
        10 ** rebels_dust_masses[:, 7],
        xerr=[
            10 ** rebels_dust_masses[:, 1]
            - 10 ** (-rebels_dust_masses[:, 3] + rebels_dust_masses[:, 1]),
            -(10 ** rebels_dust_masses[:, 1])
            + 10 ** (rebels_dust_masses[:, 2] + rebels_dust_masses[:, 1]),
        ],
        yerr=[
            10 ** rebels_dust_masses[:, 7]
            - 10 ** (-rebels_dust_masses[:, 9] + rebels_dust_masses[:, 7]),
            -(10 ** rebels_dust_masses[:, 7])
            + 10 ** (rebels_dust_masses[:, 8] + rebels_dust_masses[:, 7]),
        ],
        fmt="v",
        mec="lightskyblue",
        mfc="none",
    ...
    if np.abs(redshift - 6.0) < zprec:
        popfid = ax.plot(
            10 ** popping_17_SMDM_fid[0],
            10 ** popping_17_SMDM_fid[1],
            linestyle="--",
            linewidth=2,
            color="magenta",
        )
        ...
        pophigh = ax.plot(
            10 ** popping_17_SMDM_highcond[0],
            10 ** popping_17_SMDM_highcond[1],
            linestyle="--",
            linewidth=2,
            color="gold",
        )
        ...
        vij = ax.plot(
            10 ** vij_19[0], 10 ** vij_19[1], linestyle="--", linewidth=2, color="cyan"
        )
        lines.append(vij[0])
        labels.append("Vijayan+20, z=6")

    if 5.5 <= redshift < 6.5:
        graz = ax.fill_between(
            10 ** graziani_box[0],
            y1=graziani_dw,
            y2=graziani_up,
            linewidth=2,
