...
def plot_bouw14(ax, redshift, dir_path, lines, labels, delta_z=0.1):
    ...
    if len(ok_z_bouw) > 0:
        ok_bouwens_mags = np.asarray(bouwens14_betas[ok_z_bouw])[:, 0]
        ok_bouwens_betas_mean = np.asarray(bouwens14_betas[ok_z_bouw])[:, 1]
        ok_bouwens_betas_error = np.asarray(bouwens14_betas[ok_z_bouw])[:, 2]
